...
export default async function ExplainPage({ searchParams }: PageProps) {
  const [{ assignmentId, topic, grade }, thinkFirstEnabled, teachBackEnabled, nudgesEnabled, maxImageMb] = await Promise.all([
    searchParams,
    ...
    <div className="h-dvh bg-parchment flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto py-10 px-4">
        <div className="max-w-xl mx-auto">
          ...
          <ConceptExplainer assignmentId={assignmentId} initialTopic={topic} initialGrade={grade ? parseInt(grade) : undefined} thinkFirstEnabled={thinkFirstEnabled} teachBackEnabled={teachBackEnabled} nudgesEnabled={nudgesEnabled} maxImageMb={maxImageMb} />
        </div>
      </div>
      <SiteFooter />
    </div>
