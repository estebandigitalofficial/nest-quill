    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
      <div className="bg-adm-surface border border-adm-border rounded-2xl max-w-lg w-full p-6 space-y-4">
        ...
        <div>
          ...
          <input type="text" className="w-full bg-adm-surface border border-adm-border rounded-lg px-3 py-2 text-sm text-adm-muted" value={form.name} onChange={(e) => update('name', e.target.value)} />
        </div>

        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="text-xs text-adm-muted block mb-1">Trigger type</label>
            <select className="w-full bg-adm-surface border border-adm-border rounded-lg px-3 py-2 text-sm text-adm-muted" value={form.trigger_type} onChange={(e) => update('trigger_type', e.target.value)}>
        ...
        <div>
          <label className="text-xs text-adm-muted block mb-1">Template</label>
          <select className="w-full bg-adm-surface border border-adm-border rounded-lg px-3 py-2 text-sm text-adm-muted" value={form.template_id} onChange={(e) => update('template_id', e.target.value)}>
        ...
        <div>
          <label className="text-xs text-adm-muted block mb-1">Conditions (JSON)</label>
          <textarea className="w-full bg-adm-surface border border-adm-border rounded-lg px-3 py-2 text-sm text-adm-muted font-mono min-h-[80px] resize-y" value={form.conditions} onChange={(e) => update('conditions', e.target.value)} />
        </div>
