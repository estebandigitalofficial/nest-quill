...
export default function FrontMatterPanel({ bookId }: { bookId: string }) {
  ...
  async function load() {
    ...
    setSections((data as WriterBookSection[]).filter(s => s.zone === 'front').sort((a, b) => a.position - b.position))
    setLoading(false)
    setLoaded(true)
  }
    ...
    <div className="bg-adm-surface border border-adm-border rounded-xl overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2">
          <span className="text-xs font-bold text-adm-muted uppercase tracking-widest">Front Matter</span>
          {enabledCount > 0 && (
      ...
      {open && (
        <div className="border-t border-adm-border px-5 py-4 space-y-2">
          {loading ? (
            ...
            <>
              <p className="text-xs text-adm-subtle pb-1">Toggle sections on to include them. Click Edit to add content.</p>
